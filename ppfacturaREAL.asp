...
IF UCASE(TRIM(CTD)) <> "FT" THEN 
    response.write("esto no es una Factura")
	response.end
END IF 


'*************************************************************************
' JOLVERIN DICE QUE LO COMENTE PORQUE NO VOY A PODER HACER PRUEBAS AUN
'*************************************************************************
'IF LEFT(UCASE(TRIM(SER)),1) <> "F" THEN 
...
'CABECERA
cad =   " select * from view_guia_real_fact   " & _
    ...
    if rs.recordcount <= 0 then 
        RESPONSE.WRITE("Documento inválido")
        response.end
    END IF
    ...
    cadDet = " SELECT * FROM (select * from RSFACCAR..FT0012FACD  where F6_CTD = '"&CTD&"' AND F6_CNUMSER = '"&SER&"' AND F6_CNUMDOC = '"&DOC&"' union  " & _
             " select * from RSFACCAR..FT0012ACUD  where F6_CTD = '"&CTD&"' AND F6_CNUMSER = '"&SER&"' AND F6_CNUMDOC = '"&DOC&"') XX  where upper(F6_CCODIGO) <> upper('txt')  ORDER BY F6_CITEM "
...
''	clienteDOC = "72218501"
	...
	'IF LEN(TRIM(RSCLI("CL_CDOCIDE"))) > 0 THEN
		clienteTipDoc = TRIM(RSCLI("CL_CDOCIDE"))
	'END IF
	...
	'CONDICION PARA CLIENTES DE EXPORTACION'

	if TRIM(RSCLI("CL_CDOCIDE")) = "" and ucase(TRIM(RSCLI("CL_CTIPPER"))) = "J" then
...
f1Fecdoc = trim(AlphaNumericOnly(CStr(year(RS("F5_DFECDOC"))))   &"-"&   AlphaNumericOnly(right("00"&month(RS("F5_DFECDOC")),2))  &"-"&    AlphaNumericOnly(right("00"&day(RS("F5_DFECDOC")),2)))
	...
	'=============================================================================================================='
	nombre = ""
	f5_cnumser = rs("F5_CNUMser")
	'FILA 1 B: * Número de documento'
	 if left(F5_CNUMser,1) = "F" then 
	    f1Numdoc = ucase(trim(rs("F5_CNUMser")))  &"-"&   ucase(right(trim(rs("F5_Cnumdoc")),7))
	    NOMBRE = miRuc&"-"&coddoc&"-"&ucase(right(trim(rs("f5_cnumser")),4))&"-"&ucase(right(trim(rs("f5_cnumdoc")),7))
    else
        f1Numdoc = "F" & ucase(trim(rs("F5_CNUMser")))  &"-"&   ucase(right(trim(rs("F5_Cnumdoc")),7))
        NOMBRE = miRuc&"-"&coddoc&"-"&ucase("F"&right(trim(rs("f5_cnumser")),4))&"-"&ucase(right(trim(rs("f5_cnumdoc")),7))
    end if
	...
	IF tipoperacion = "0200" THEN
		f1SumMontoBaseIGV="0.0"
		f1ImporteIGV="0.0"
	END IF
	...
	if isnull(RS("C5_CNUMDOC")) and len(trim(rs("F5_CGLOSA"))) = 0 then
		cntGuias = ""
	else
		cntGuias = 1
	end if
	f1CantGuias									=cntGuias
	...
	f1CantPuntPartiLlegada						=cntGuias
	...
	if tipoperacion = "0200" then
		f1MontoTotalImpuestos= "0"
	end if


	'FILA 1 BC: Total Valor de Venta 
	f1TotalValorVenta							=TOTAL
...
f1TotalDescuentosAB			=FORMATNUMBER(CDBL(rsIGV("dcto")),2,,,FALSE)

if CDBL(rsIGV("dcto")) = 0 then
f1TotalDescuentosAB = ""
end if
	...
	'FILA 2 A:Código ubigeo punto partida 
	f2CodUbigeo 							= ""
	'FILA 2 B:Dirección punto partida 
	drdefecto = "AV.LA MAR NRO.523"

	if tipoperacion = "0200" then
		drdefecto = ""
	end if

	f2DireccionPuntoPartida 				= replace(AlphaNumericOnly(trim(drdefecto)) & " Pto. Llegada: "&AlphaNumericOnly(trim(rs("c5_cdirenv"))),"  "," ")
	
	if tipoperacion = "0200" then
		f2DireccionPuntoPartida = ""
	end if

	'FILA 2 C:Urbanizacion punto partida 
	f2UrbanizacionPuntoPartida              = ""
	'FILA 2 D:Provincia punto partida
	f2ProvinciaPuntoPartida 				= ""
	'FILA 2 E:Departamento punto partida
	f2DepartamentoPuntoPartida 				= ""
	'FILA 2 F:Distrito punto partidA     
	f2DistritoPuntoPartida 					= ""
	'FILA 2 G:Código país punto PARTIDA
	f2CodPaisPuntoPartida 					= ""


	'Fila 2 H :Código ubigeo punto llegada 
	Fila2H									=""
	'Fila 2 I :Dirección punto llegada 
	Fila2I									=AlphaNumericOnly(rs("c5_cdirenv"))
	...
	if isnull(rs("tr_cruc")) then
		Fila2T									=""
	else
		Fila2T									="6"
	end if
	'Fila 2 U :Razón social del transportista 
	Fila2U									=AlphaNumericOnly(rs("tr_cnombre"))
	...
	Fila2AE									=AlphaNumericOnly(RS("TR_CNOMFER"))
	...
	fila2=f2CodUbigeo
	fila2 = fila2 &","&f2DireccionPuntoPartida
	fila2 = fila2 &","&f2UrbanizacionPuntoPartida
	fila2 = fila2 &","&f2ProvinciaPuntoPartida
	fila2 = fila2 &","&f2DepartamentoPuntoPartida
	fila2 = fila2 &","&f2DistritoPuntoPartida
	fila2 = fila2 &","&f2CodPaisPuntoPartida
	fila2 = fila2 &","&Fila2H	
	fila2 = fila2 &","&Fila2I	
	fila2 = fila2 &","&Fila2J	
	fila2 = fila2 &","&Fila2K	
	...
	f4NroGuia 								= LEFT("0"&TRIM(RS("C5_CNUMDOC")),4)&"-"&RIGHT(TRIM(RS("C5_CNUMDOC")),7)

	if len(TRIM(RS("F5_CGLOSA"))) > 0 then
		f4NroGuia=trim(RS("F5_CGLOSA"))
	end if

	if operacion = "0200" then
	...
	if isnull(RS("C5_CNUMDOC")) and len(trim(RS("C5_CNUMDOC")))=0 or tipoperacion= "0200" then 
		f4NroGuia= ""
		f4CodGuia= ""
		f4NroOtroDoc= ""
		f4CodOtroDoc= ""
	...
	MONLET = " SOLES"
	IF UCASE(TRIM(RS("F5_CCODMON"))) = "US" THEN
		MONLET = " DOLARES AMERICANOS"
	END IF

	f71000 = Numlet(cdbl(total))&MONLET
	f71002 = ""
	f72000 = ""
	f72001 = ""
	f72002 = ""
	...
	if rsDet.recordcount = 0 then
		response.write("No hay lineas que registrar")
		response.end
	end if
	rsDet.movefirst
	FOR I = 0 TO rsDet.recordcount-1 
		...
		'if cdbl(rsDet("F6_NCANTID")) < 1 then
	''		response.write("Las unidades vendidas no pueden ser 0 !!")
	''		response.end
''		end if

		'if ucase(trim(rsDet("f6_ccodigo"))) <> "TXT" AND ucase(trim(rsDet("f6_ccodigo"))) <> "."  then
			DESCRIP = ""
			DESCRIP = trim(AlphaNumericOnly(ucase(replace(replace(trim(rsDet("f6_cdescri")),",",""),"/"," "))))
			...
			if rsTXT.recordcount > 0 then
				...
				for txtcont = 0 to rsTXT.recordcount-1 
					DESCRIP = DESCRIP & trim(rsTXT("F6_CDESCRI"))
					rsTXT.movenext
				next
			end if

			if I = rsDet.recordcount-1  then
				cadDetCant = " SELECT isnull(sum(f6_ncantid),0) as sumCanTid FROM (select * from RSFACCAR..FT0012FACD  where F6_CTD = '"&CTD&"' AND F6_CNUMSER = '"&SER&"' AND F6_CNUMDOC = '"&DOC&"' union  " & _
			...
			cadAlternativos = "select AT_CCODAL from RSFACCAR..AL0012CODA where AT_CCODIGO='"&ucase(trim(rsDet("f6_ccodigo")))&"' and AT_CCODCLI='"&CODCLI&"'"
			'response.write(cadAlternativos)
			set rsAlter = RsNuevo

			if rsAlter.state > 0 then
				rsAlter.close
			end if

			'ABRIMOS RECORDSET'
			rsAlter.open cadAlternativos,cnn

			IF rsAlter.recordcount > 0 THEN 
				rsAlter.movefirst
				FILA9AU = trim(rsAlter("AT_CCODAL"))
			END IF
			...
			fDetalleNroOrden 							= i+1'cint(rsDet("F6_CITEM"))
			fDetalleUnidMed								= "C62"		'APLICANDO CODIGO INTERNACIONAL'		
			fDetalleCantid								= cdbl(rsDet("f6_ncantid"))
			fDetalleDescrip								= DESCRIP

			fDetallePreVentUnit							= formatnumber( ( CDBL(rsDet("f6_nimpmn"))   ) / CDBL(rsDet("f6_ncantid")),2,,,false )

			fDetalleMontoBaseIgv						= formatnumber(CDBL(rsDet("f6_nimpmn")) - CDBL(rsDet("f6_nigv")),2,,,false)
			
			fDetalleMontoTotalIgv						= formatnumber(CDBL(rsDet("f6_nigv")),2,,,false) 
			fDetalleTipoAfectIgv						= "10"			
			fDetalleCodigoTibutos						= "1000"
			fDetalleProcentIgv							= formatnumber(cdbl(rsDet("f6_nigvpor")),0,,,false)

			
			fDetalleCodProd 							= ucase(trim(rsDet("f6_ccodigo")))
			fDetalleValUnit								= formatnumber( ( CDBL(rsDet("f6_nimpmn"))  + CDBL(rsDet("f6_ndescto")) - CDBL(rsDet("f6_nigv"))  ) / CDBL(rsDet("f6_ncantid")),2,,,false )
			fDetalleValVent								= formatnumber(CDBL(fDetalleValUnit)*cdbl(fDetalleCantid) - cdbl(rsDet("f6_ndescto")),2,,,false)


			if tipoperacion = "0200" and MON = "USD" then
			
				fDetallePreVentUnit 						= formatnumber( ( CDBL(rsDet("F6_NIMPUS"))   ) / CDBL(rsDet("f6_ncantid")),2,,,false )
				fDetalleMontoBaseIgv						= formatnumber(CDBL(rsDet("F6_NIMPUS")),2,,,false)
				fDetalleMontoTotalIgv						= "0"
				fDetalleTipoAfectIgv						= "40"			
				fDetalleCodigoTibutos						= "9995"
				fDetalleProcentIgv							= "0"
				fDetalleValUnit								= formatnumber( ( CDBL(rsDet("F6_NIMPUS"))  + CDBL(rsDet("f6_ndescto")) - CDBL(rsDet("f6_nigv"))  ) / CDBL(rsDet("f6_ncantid")),2,,,false )
				fDetalleValVent								= formatnumber(CDBL(fDetalleValUnit)*cdbl(fDetalleCantid) - cdbl(rsDet("f6_ndescto")),2,,,false)
			end if


			'SOLO APARECERA SI HAY DESCUENTO
			if cdbl(rsDet("f6_ndescto")) > 0 then
				...
				fDetalleMontoBaseDescuentoAB 				= formatnumber(cdbl(rsDet("f6_ndescto"))  - CDBL(rsDet("f6_nigv"))  + cdbl(rsDet("f6_nimpmn")  ),2,,,false)
				fDetalleFactorDescuentoAB					= formatnumber(cdbl(rsDet("f6_npordes"))/100,2,,,false)
				fDetalleMontoTotalDescuentoAB				= formatnumber(cdbl(rsDet("f6_ndescto")),2,,,false)
				FILA9AP										= formatnumber(cdbl(rsDet("f6_npordes")),2,,,false)
			end if
			
	        fDetalleMontTotalImp						= formatnumber(CDBL(rsDet("f6_nigv")),2,,,false)
			fDetalleTotaldelaLinea						= fDetalleMontoBaseIgv

			if ucase(left(trim(rsDet("f6_ccodigo")),3)) = "BOL" then
				fDetalleMontTotalImp = formatnumber(CDBL(rsDet("f6_nigv")),2,,,false) + cdbl(fDetalleMontoTotalIgv)
				fDetalleTotaldelaLinea = cdbl(fDetalleMontTotalImp) + formatnumber(CDBL(rsDet("f6_nimpmn")),2,,,false)
		...
		'else
			...
			'DESCRIP = ""
			'DESCRIP = trim(utf8_simbom(ucase(replace(replace(trim(rsDet("f6_cdescri")),",",""),"/"," "))))
			'IF DESCRIP = "" THEN
			'	DESCRIP = "."
			'END IF
			'fDetalleNroOrden 							= cint(rsDet("F6_CITEM"))
			'fDetalleUnidMed								= TRIM(rsDet("F6_CUNIDAD"))
			'fDetalleCantid								= cdbl(rsDet("f6_ncantid"))
			'fDetalleDescrip								= DESCRIP
'
'			'fDetallePreVentUnit							= 0
'
'			'fDetalleMontoBaseIgv						= 0
'			'
'			'fDetalleMontoTotalIgv						= 0
'			'fDetalleTipoAfectIgv						= 10			
'			'fDetalleCodigoTibutos						= "1000"
...
'			'fDetalleValVent								= 0
'			'
'	        'fDetalleMontTotalImp						= 0
			'fDetalleTotaldelaLinea						= 0
	...
	cnn.execute("insert into REAL_POS values('"&operacion&"','"&CTD&"','"&SER&"','"&DOC&"');")

	
	set fs=Server.CreateObject("Scripting.FileSystemObject")
	fs.MoveFile "d:\EFACT_MODULO\daemon\documents\in\"&nombre&".csv","d:\EFACT_MODULO\daemon\documents\in\invoice\"&nombre&".csv"
	set fs=nothing
	...
	Response.Redirect "/apijf/public/index.php/auth?tipdoc=invoice&ope="&operacion&"&file="&nombre&".xml"
