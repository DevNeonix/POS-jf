...
marka = 0
    ...
    'response.write(cad)
    cnn.execute cad
cad =             " select  CODIGO, DESCRI, [07] AS [AS],[08] AS PO, [09] AS CH, [10] AS SB, [11] AS FB, [12] as SI, [15] as TR2,[14] as AR from #temp   " & _
            " pivot (SUM(UME) for tienda in ([01],[07],[08],[09],[10],[11],[12],[15],[14])) as MIN       " & _
			" order by codigo; "
            response.write(cad)
           ' response.write("<br>")
    RS.OPEN CAD,CNN
    ...
    <%IF  RS.EOF THEN response.end%>
        <%DO WHILE NOT RS.EOF%>                       
            ...
            <%do while trim(codigo) = trim(rs.fields.item(0)) and not rs.eof%>
                    ...
                    <%if not ISNULL(rs("TR2")) then if cint(rs("TR2")) > 0 then aCan(6) = CINT(rs("TR2")) ELSE ACAN(6) = 0%>
                    <%if not ISNULL(rs("AR")) then if cint(rs("AR")) > 0 then aCan(7) = CINT(rs("AR")) ELSE ACAN(7) = 0%>
                    <%rs.movenext%>
                    <%if rs.eof then exit do%>
            <%loop%>
        <%IF CINT(ACAN(0))+CINT(ACAN(1)) +CINT(ACAN(2)) + CINT(ACAN(3)) + CINT(ACAN(4)) + CINT(ACAN(5)) + CINT(ACAN(6)) + CINT(ACAN(7)) > 0 THEN %>
                ...
                <%FOR I=0 TO 7 %>
                 <td class="Estilo5" align="center"  width="5%">
                    <%IF CINT(ACAN(I))> 0 THEN%> 
                      <input id="<%=MID(LETRAS,I+1,1)&CONT%>"   value="<%=aCan(i)%>" class="Estilo11"  />
                    <% ELSE 
                        RESPONSE.WRITE("")
                    end if%>
                 </td>
