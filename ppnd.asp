...
if rs.recordcount = 0 then 
	response.write("esto no es una Nota de devito")
	response.end
end if
rs.movefirst

'VALIDANDO SI ES UNA BOLETA'
'if trim(rs("coddoc")) <> "ND" then
	'response.write("esto no es una Nota de devito")
	'response.end
'end if
...
'FILA 3 A:Apellidos y nombres,denominación o razónsocial
f3RazonSocial 							=miRS
'FILA 3 B:Nombre comercial 
f3NomComercial 							=miNombreComercial
'FILA 3 C:Número de RUC 
f3NumRuc	 							=miRuc
'FILA 3 D:Código Ubigeo 
f3CodUbigeo 							=codUBIGUEO
'FILA 3 E:Dirección 
f3Direccion 							=miDireccion
'FILA 3 F:Urbanización 
f3Urbanizacion 							=""
'FILA 3 G:Departamento 
f3Departamento 							="LIMA"
'FILA 3 H:Provincia 
f3Provincia 							="LIMA"
'FILA 3 I:Distrito 
f3Distrito 								="ATE"
'FILA 3 J:Codigo del pais 
f3CodPais	 							="PE"
'FILA 3 K:Código delestablecimiento
f3CodEst	 							="0000"
...
if trim(rs("docori")) = "BL" then
	...
	oriTipDoc	= "3"
	cadOri = "EXEC SP_DOCORI 'BL"&left(trim(rs("NUMORI")),3)&right(trim(rs("NUMORI")),7)&"'"
...
elseif trim(rs("docori")) = "FC" then
	...
	oriNumDoc	= right(trim(rs("NUMORI")),7)
	oriTipDoc	= "1"
	cadOri = "EXEC SP_DOCORI 'FC"&left(trim(rs("NUMORI")),3)&right(trim(rs("NUMORI")),7)&"'"
...
FILA7B = oriTipDoc
'FILA 7  C:Codigo de tipo de nota de debito
FILA7C = "02"
'FILA 7  D:Motivo de tipo de nota de debito
FILA7D = "Aumento en el valor"
...
set f=fs.CreateTextFile("d:\efact\daemon\documents\in\debitnote\"&nombre&".csv",true)
...
Response.Redirect "/apijf/public/index.php/auth?tipdoc=debitnote&ope="&ope&"&file="&nombre&".xml"
