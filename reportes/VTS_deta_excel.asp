...
CAD =   " SET DATEFORMAT DMY                                                            " & _
        " select  tienda,ltrim(rtrim(CODART)) CODART, DESCRI, SUM (SALE) AS CANT, MAX(LISTA1*1.18) AS UNIT,         " & _
        " SUM(PVP-IGV) AS PVP, AVG(PORDES) AS PORDES, SUM(DCT) AS DCT, SUM(IGV) AS IGV, " & _
        "  SUM(PVP-IGV) -  SUM(DCT) +  SUM(IGV) AS TOT                                  " & _
        " FROM VIEW_VENTAS_ARTICULO_2 WHERE SALE > 0  and isnull(pvp,0) > 0               " & _
