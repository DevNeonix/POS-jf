...
<script type="text/jscript" language="jscript">
    function strSQL() {
        ...
        cad = "ctodeta.asp?TDA=" + document.all.tienda.value + '&ini=' + document.all.ini.value + '&fin=' + document.all.fin.value + '&tempo=' + document.all.tempo.value
...
<%
                set rstempo = RsNuevo
                rstempo.open "select * from temporadas" , cnn
            %>
...
<table width="100%">
    <tr><td align="center" class="Estilo6">Costos  x Grupo de Articulos :</td></tr>
</table>
...
<table id="Table1" align="center"  bordercolor="#FFFFFF"  bgcolor="<%=Application("color2")%>"  cellpadding="2"  cellspacing="1"  border="0">
	<tr valign="middle" >
        <td class="Estilo11" valign="middle" align="right" rowspan="2">
            <label for="Radio">Temporada:&nbsp;</label></td> 
        <td>
            <select  name="tempo" id="tempo">
                <option value = "TT" selected>TODAS</option>
                <%do while not rstempo.eof %>
                    <option value="<%=TRIM(rstempo("CODIGO"))%>"><%=TRIM(rstempo("DESCRIPCION")) %></option>
                    <%rstempo.movenext %>
                <%loop %>
                <%rstempo.close %>
            </select>
