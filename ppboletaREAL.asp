...
<!--#include file="./comun/funcionesefact.asp"-->
<%  
...
'*************************************************************************
'IF LEFT(UCASE(TRIM(SER)),1) <> "B" THEN 
'    response.write("esto no es una boleta")
'	response.end
'END IF 
'*************************************************************************
...
'RESPONSE.END
	...
	'=============================================================================================================='
	'=============================================================================================================='
	nombre = miRuc&"-"&coddoc&"-"&ucase("B"&right(trim(rs("f5_cnumser")),3))&"-"&ucase(right(trim(rs("f5_cnumdoc")),7))
    ...
    cadIGV = "select  sum(igv) as igv, sum(DCTO) AS DCTO from (Select F6_NIGV as igv, F6_NDESCTO AS DCTO From RSFACCAR..FT0012ACUD Where F6_CCODAGE= '0001' AND F6_CTD ='"&ctd&"' AND F6_CNUMSER='"&ser&" ' AND F6_CNUMDOC='"&doc&"' " & _
             " UNION Select F6_NIGV as igv, F6_NDESCTO AS DCTO From RSFACCAR..FT0012FACD Where F6_CCODAGE= '0001' AND F6_CTD ='"&ctd&"' AND F6_CNUMSER='"&ser&" ' AND F6_CNUMDOC='"&doc&"' ) as igvdes"
'response.write(cadIGV)
    ...
    IF UCASE(TRIM(RS("F5_CCODMON"))) = "MN" THEN 	
    	MON = "PEN" 
    ELSE 
    	MON = "USD"
    end if
...
''	clienteDOC = "72218501"
	...
	if ucase(clienteDOC) = "N" then
		clienteTipDoc		= "1"
        NumDocCLI = trim(rsCLI("cl_cnumide"))
	elseif ucase(clienteDOC) = "J" then
		clienteTipDoc		= "6"
        NumDocCLI = trim(rsCLI("cl_cnumruc"))
	ELSE
		if cint(NumDocCLI) = 0 then
			clienteTipDoc		= "0"
		    NumDocCLI = trim(rsCLI("cl_cnumide"))
		end if
	end if
	...
	f3SerieCorre 							=""
	'FILA 3 E: Fecha emisiónanticipo
	f3FechEmi 								=""
	...
	if cdbl(rsDet("F6_NCANTID")) < 1 then
		...
		if ucase(left(trim(rsDet("f6_ccodigo")),3)) = "BOL" then
			...
			FILA9EV			= "0.2"
			FILA9EW			= CDBL("0.2") * CDBL(fDetalleCantid)
		end if
		...
		IF rsAlter.recordcount > 0 THEN 
			...
			fDetalleDescrip = fDetalleDescrip &"...."&trim(rsAlter("AT_CCODAL"))&"...."
		END IF
	...
	set fs=Server.CreateObject("Scripting.FileSystemObject")
	fs.MoveFile "d:\EFACT_MODULO\daemon\documents\in\"&nombre&".csv","d:\EFACT_MODULO\daemon\documents\in\boleta\"&nombre&".csv"
	set fs=nothing



	Response.Redirect "/apijf/public/index.php/auth?tipdoc=boleta&ope="&operacion&"&file="&nombre&".xml"
