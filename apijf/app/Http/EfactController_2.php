        ...
        try {
            ...
            if ($requestAuth->getStatusCode() >= 200 && $requestAuth->getStatusCode() < 300) {
                ...
                $requestDownload = $client->request("GET", "$url", [
                    ...
                    'save_to' => "\\\\intranet2\\documentos\\efact\\$ticket.$tipo2",
//                    "form_params" => ["username" => $user, "password" => $password, "grant_type" => "password"]
                ]);
                //return  ($bearer);

                return Response::download("\\\\intranet2\\documentos\\efact\\$ticket.$tipo2")->deleteFileAfterSend(true);
...
//                    "form_params" => ["username" => $user, "password" => $password, "grant_type" => "password"]
                ...
                //return  ($bearer);
                if(!empty($download)){
                    return Response::download("\\\\intranet2\\documentos\\efact\\$ticket.$tipo2");
                }
                return Response::file("\\\\intranet2\\documentos\\efact\\$ticket.$tipo2")->deleteFileAfterSend(true);
