...
class EfactController extends Controller
{
    ...
    public function authweb(Request $request)
    {
        ...
        $user = "fajas_elasticos@textiles.com";
        $password = "MnvzF0x8";
        $credential64 = "Basic Y2xpZW50OnNlY3JldA==";
        $contentType = ['content-type' => 'application/json'];

        $url="https://ose-gw1.efact.pe/core-ui/auth/login";
        ...
        try {
            ...
            $requestAuth = $client->request("POST", $url, [
                ...
                "body" => '{
                    "username":"fajas_elasticos@textiles.com",
                    "password":"MnvzF0x8"
                }'
            ]);
    ...
    public function sendFile(Request $request)
    {
        ...
        $fullPath = "\\\\intranet\\D\\EFACT_MODULO\\daemon\\documents\\out\\$tipdoc\\" . $request->input("file");

        $ferr =  str_replace(".xml","","\\\\intranet\\D\\EFACT_MODULO\\daemon\\documents\\err\\$tipdoc\\" . $request->input("file")).".csv";
    ...
    public function baja(Request $request)
    {
        ...
        try {
            ...
            if ($requestAuth->getStatusCode() >= 200 && $requestAuth->getStatusCode() < 300) {
                $response = json_decode($requestAuth->getBody(), true);
                $token = $response["access_token"];
                $fullPath = "\\\\intranet\\D\\EFACT_MODULO\\daemon\\documents\\in\\" . $request->input("file");
...
//                return $requestSendFile->getBody();
                        ...
                        sleep(1);
                        return "<p>Baja: ".$ticket." Se ha eliminado de la Sunat, regresando stocks...</p><script>setTimeout(function(){window.close();},1000)</script>";
                    }
                } catch (\GuzzleHttp\Exception\RequestException $e) {
        ...
        try {
            ...
            if ($requestAuth->getStatusCode() >= 200 && $requestAuth->getStatusCode() < 300) {
                ...
                $requestDownload = $client->request("GET", "$url", [
                    ...
                    'save_to' => "\\\\intranet\\D\\ventas_new\\efact\\$ticket.$tipo2",
//                    "form_params" => ["username" => $user, "password" => $password, "grant_type" => "password"]
                ]);
                //return  ($bearer);

                return Response::download("\\\\intranet\\D\\ventas_new\\efact\\$ticket.$tipo2")->deleteFileAfterSend(true);
...
//                    "form_params" => ["username" => $user, "password" => $password, "grant_type" => "password"]
                ...
                if(!empty($download)){
                    return Response::download("\\\\intranet\\D\\ventas_new\\efact\\$ticket.$tipo2");
                }
                return Response::file("\\\\intranet\\D\\ventas_new\\efact\\$ticket.$tipo2");
...
//                    "form_params" => ["username" => $user, "password" => $password, "grant_type" => "password"]
                ...
                if ($requestDownload->getStatusCode() >= 200 && $requestDownload->getStatusCode() <= 299) {
                    ...
                    return Redirect::route('show', ["ticket"=>$tt,"tipo"=>"pdf"]);
                    //return response()->redirect(route('show',["ticket"=>$tt,"tipo"=>"pdf"]));
