...
if rs.recordcount > 0 then 
	...
	if trim(rs("coddoc")) <> "BL" then
		response.write("esto no es una boleta")
		response.end
	end if

	cadDet = "select distinct d.*,l.AR_CDESCRI as descri from JACINTA..movimdet d full outer join rsfaccar..AL0012ARTI l on replace(d.CODART,' ','') = l.AR_CCODIGO where OPERACION='"&ope&"' "
	...
	'=============================================================================================================='
	'=============================================================================================================='
	nombre = miRuc&"-"&coddoc&"-"&ucase("B"&right(trim(rs("serie")),3))&"-"&ucase(right(trim(rs("numdoc")),7))
...
' PARA CLIENTES CON CARNET DE EXTRANJERIA MM--> 23-12-2019
	...
	'***********CLIENTE VARIOS*******'
    if trim(clienteDOC) = "00000000" or trim(clienteDOC) = "00000000000" then
		clienteTipDoc		= "0"
	end if
	...
	f1Numdoc = ucase("B"&right(trim(rs("serie")),3))&"-"&ucase(right(trim(rs("numdoc")),7))
	...
	set f=fs.CreateTextFile("d:\EFACT_MODULO\daemon\documents\in\boleta\"&nombre&".csv",true)
	...
	if cdbl(rsDet("SALE")) < 1 then
		...
		fDetalleDescrip								= AlphaNumericOnly(ucase(replace(replace(trim(rsDet("descri")),",",""),"/"," ")))
		...
		if trim(rs("tienda")) = "06" then
			fDetalleMontoBaseIgv		= 	formatnumber(CDBL(rsDet("PRECIO")) - CDBL(rsDet("igv")) ,2,,,false)
			fDetalleValUnit				= 	formatnumber(cdbl(CDBL(rsDet("PRECIO")) - CDBL(rsDet("igv")) )/cdbl(fDetalleCantid),2,,,false)
			fDetalleValVent				= 	formatnumber(CDBL(rsDet("PRECIO")) - CDBL(rsDet("igv")) ,2,,,false)
		end if
	...
	Response.Redirect "/apijf/public/index.php/auth?tipdoc=boleta&ope="&ope&"&file="&nombre&".xml"
