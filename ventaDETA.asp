...
<table id="Table1" align="center"  bordercolor="#FFFFFF"  bgcolor="<%=Application("color2")%>"  cellpadding="0"  cellspacing="1"  border="1" align="center" >
   <tr>
    ...
    <tr id="lin<%=i%>" name="lin<%=i%>" <%IF i>0 THEN %>style="display:none"<%END IF %>>
        ...
        <td><input id="COD<%=i%>" name="COD<%=i%>" size="20" class="Estilo24"  maxlength="25" onchange="this.value=this.value.toUpperCase();carga('<%=i%>');stock('<%=i%>')" value='' ondblclick="hlp('<%=i%>', this)" /></td>
...
function carga(cn) {
    ...
    if ((ltrim(rtrim(eval("document.all.COD" + cn + ".value.toUpperCase()")))) == 'SALDO00000' || (ltrim(rtrim(eval("document.all.COD" + cn + ".value.toUpperCase()")))) == 'FLETE') {
        //alert()
...
function hlp(op, obj) {
   ...
   PopupCenter('help/hlpprendasVT.asp?pos='+trim(vobj)+'&op='+op,500,500)
    return true;
}
function stock(op) {
    aDisp = new Array();
