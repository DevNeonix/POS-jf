...
cad =             " select  CODIGO, DESCRI, isnull([07],0)AS  [AS],isnull([08],0) AS PO,isnull([09],0) AS CH," & _
                  "  isnull([10],0) AS SB, isnull([12],0) as SI,  isnull([14],0) as AR,  isnull([15],0) as T2,isnull([16],0) as 'OUT'   from #temp   " & _
            " pivot (SUM(stock) for tienda in ([01],[07],[08],[09],[10],[12],[14],[15],[16])) as STK       " & _
    ...
    <%IF  RS.EOF THEN response.end%>
        <%DO WHILE NOT RS.EOF%>                       
            ...
            <%do while trim(codigo) = trim(rs.fields.item(0)) and not rs.eof%>
                    ...
                    <%if not ISNULL(rs("si")) then if cint(rs("SI")) > 0 then aCan(4) = CINT(rs("SI")) ELSE ACAN(4) = 0 %>
                    <%if not ISNULL(rs("AR")) then if cint(rs("AR")) > 0 then aCan(5) = CINT(rs("AR")) ELSE ACAN(5) = 0 %>
                    <%if not ISNULL(rs("T2")) then if cint(rs("T2")) > 0 then aCan(6) = CINT(rs("T2")) ELSE ACAN(6) = 0 %>
                    <%if not ISNULL(rs("OUT")) then if cint(rs("OUT")) > 0 then aCan(7) = CINT(rs("OUT")) ELSE ACAN(7) = 0 %>
                    <%rs.movenext %>
                    <%if rs.eof then exit do%>
            <%loop%>
        <%IF CINT(ACAN(0))+CINT(ACAN(1)) +CINT(ACAN(2)) + CINT(ACAN(3)) + CINT(ACAN(4))  + CINT(ACAN(5)) + CINT(ACAN(6)) +  CINT(ACAN(7))+ CINT(ACAN(8)) +CINT(ACAN(9))  THEN %>
